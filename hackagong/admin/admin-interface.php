...
public static function optionsframework_machine($options) {
	...
	foreach ($options as $value) {
		...
		case 'slider':
			...
			$output .= '</ul>';
			$output .= '<a href="#" class="button slide_add_button">Add New Widget</a></div>';
...
public static function optionsframework_slider_function($id,$std,$oldorder,$order,$int){
	...
	if (!empty($val['title'])) {
		...
		$slider .= '<li><div class="slide_header"><strong>Widget '.$order.'</strong>';
	}
	...
	$slider .= '<input class="slide of-input" name="'. $id .'['.$order.'][link_text]" id="'. $id .'_'.$order .'_slide_link_text" value="'. $val['link_text'] .'" />';
