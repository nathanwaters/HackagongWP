...
class sf_flickr_widget extends WP_Widget {
	...
	function sf_flickr_widget() {	
		...
		$this->WP_Widget( 'flickr-widget', 'Flickr', $widget_ops);
	}
...
<?php
	...
	function widget($args, $instance) {
		...
		require_once(TEMPLATEPATH . "/includes/widgets/phpFlickr/phpFlickr.php");
		$f = new phpFlickr($flickr_api); //Insert API key here
		$f->enableCache("fs", TEMPLATEPATH ."/includes/widgets/cache/");
		...
		if (!empty($flickr_id)) {
			...
			foreach ((array)$photos['photos']['photo'] as $photo) {
				echo "<li><a class='flickr-img-link' href=$photos_url$photo[id]>";
