		...
		if (!file_exists($cachefile) || (file_exists($cachefile) && (filemtime($cachefile) + $interval) < time())) :
			...
			if (method_exists($feed, 'get_items')) :
				...
				foreach	($tweets as $t) :
					...
					$result .= '
							<div class="twitter-text">' . substr($text, $prefixlen, strlen($text) - $prefixlen) . '</div><span class="tweet-arrow"> </span>';
				...
				$result .= '<div class="twitter-link">Follow <a href="http://www.twitter.com/'. $username .'">@'. $username .'</a> on Twitter.</div>'; 
